-- Expose machine-readable bounds for safe time-bucket drill selection.
drop function if exists public.dashboard_trend(text,date,date,text,jsonb);
...
returns table(period text,bucket_start date,bucket_end date,count bigint)
...
begin
 ...
 expr:=case when p_metric='new_hires' then 'e.joining_date' else 'e.leaving_date' end; pred:=public.dashboard_metric_where(p_metric,p_from,p_to); fw:=public.dashboard_filter_where(p_filters);
 ...
 fmt:=case p_granularity when 'day' then 'Mon DD' when 'week' then '"Week of "Mon DD' when 'month' then 'Mon YYYY' when 'quarter' then '"Q"Q YYYY' else 'YYYY' end;
 return query execute format('with buckets as (select date_trunc(%L,d)::date bucket from generate_series(date_trunc(%L,$1::timestamp),date_trunc(%L,$2::timestamp),%L::interval) d), counts as (select date_trunc(%L,%s)::date bucket,count(*)::bigint n from public.employees e where (%s) and (%s) group by 1) select to_char(b.bucket,%L),greatest(b.bucket,$1::date),least((b.bucket+%L::interval-interval ''1 day'')::date,$2::date),coalesce(c.n,0)::bigint from buckets b left join counts c using(bucket) order by b.bucket',p_granularity,p_granularity,p_granularity,step,p_granularity,expr,pred,fw,fmt,step) using p_from,p_to;
end $$;
